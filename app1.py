...
def predict():
    ...
    temp[0]=request.form.get('a')
    temp[1]=request.form.get('b')
    temp[2]=request.form.get('c')
    temp[3]=request.form.get('d')
    temp[4]=request.form.get('e')
    for i in range(len(temp)):
    	input_vector[temp[i]] = 1
    ...
    # return jsonify({"data":list(model.predict([input_vector])[0])})
